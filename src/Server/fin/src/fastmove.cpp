/*
This Code is for Fast Velocity
*/
#include <ros/ros.h>
...
float turn(1.4); // Angular velocity (rad/s)
...
{
  int cnt = 0;
  for(int i = 600-150; i <=600-150+val-1; i++){//DRX
    cnt += bool(Line.at<uchar>(row, i));
  }
  int max = cnt;
  int drx = 600-150;
  for(int i = 600-150+1; i <= 600-val ; i++)
  {
    cnt += bool(Line.at<uchar>(row, i+val-1));
    cnt -= bool(Line.at<uchar>(row, i-1));
...
{
  int cnt = 0;
  for(int i = 600-150; i <=600-150+val-1; i++){//URX
    cnt += bool(Line.at<uchar>(0, i));
  }
  int max = cnt;
  int urx = 600-150;
