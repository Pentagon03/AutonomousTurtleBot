...
int traffic_light()
{
  ...
  Mat mat_traffic(frame, Rect(Point(480, 120), Point(560, 360))); //실험
  imshow("mat_traffic", mat_traffic);
  Mat bgr;
  cvtColor(mat_traffic,bgr,COLOR_HSV2BGR);
  int max_row=0,max=0;
  for(int i = 0; i < bgr.rows; i++)
  {
    ...
    {
      uchar b = bgr.at<Vec3b>(i,j)[0];
      uchar g = bgr.at<Vec3b>(i,j)[1];
      uchar r = bgr.at<Vec3b>(i,j)[2];
      if(b>210 && g>210 && r>210){
        cnt++;
      }
    }
    if(cnt>max){
