...
void msgCallback(const which_way::Res_Key::ConstPtr& Res_Key){
  dbg();
  if((clock()-cur_time)*CLOCKS_PER_SEC > 1){
    ...
    key=Res_Key->key;
    // Get the pressed key
    // while((clock()-cur_time)/CLOCKS_PER_SEC<1);
    // key = ch[cur_key];
    // cur_key=(cur_key+1)%4;
...
int main(int argc, char** argv)
{
  ...
  dbg(); dbg();
  ros::Subscriber sub = nh.subscribe("Res_Key", 100, msgCallback);
  ros::spin();
  return 0;
}
